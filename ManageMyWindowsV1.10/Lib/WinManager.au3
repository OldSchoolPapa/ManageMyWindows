#cs ----------------------------------------------------------------------------
 ...
 Script Function:
    ...
    - Move the windows to a specific location with the Possibility to remove the borders and titlebars.
...
Func _MoveWindowsToSlots($aHwnds, $aWinPos, $Width, $Height, $RBTB, $WinSizeW, $WinSizeH)
			...
			Local $sEmptySlot = _ArraySearch($aWinPos, "Empty", 0, 0, 0, 1, 1, 2)
			      ...
			      If $sEmptySlot <> -1 Then
					 ...
					 If $RBTB = 1 Then
						...
						WinMove($aHwnds[$i][1], "", $aWinPos[$sEmptySlot][0], $aWinPos[$sEmptySlot][1], $WinSizeW, $WinSizeH)

						_RemoveTitleBorders($aHwnds[$i][1], $Width , $Height)
