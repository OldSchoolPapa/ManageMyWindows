...
Local $UseContainer           = GUICtrlCreateCheckbox( "Use Container GUI", 545, 100, 190, 20)
...
For $i = 1 To $aDisplaySetup[0][0]
   _ArrayAdd($aScreenCB, GUICtrlCreateCheckbox("Screen " & $i & " Resolution : " & $aDisplaySetup[$i][3] & "x" & $aDisplaySetup[$i][4] & " Pixels", 10, $y, 295, 25))
...
Func _DetectSize()
   ...
   ElseIf StringLen($WinT) >= 2 Then
	  ...
	  Return
   Else
      MsgBox(0,"Error","Input Box Empty or Text Length < to 2 Chars.")
	  Return
   EndIf
...
EndFunc
 ...
 Func _ManageWindows()
   ...
   If $Win2Search = "Title Bar(s)" Or StringLen($Win2Search) < 2 Then
	...
	While 1
	   ...
	   If BitAND(GUICtrlRead($UseContainer), $GUI_CHECKED) = $GUI_CHECKED Then
		    If $aHwnds <> @error Then
			    $aWinPos = _MoveWindowsToSlots($aHwnds, $aWinPos, $ClientSize[1], $ClientSize[2], 1)
			EndIf

		 Else
			If $aHwnds <> @error Then
			    $aWinPos = _MoveWindowsToSlots($aHwnds, $aWinPos, $WinSize[1], $WinSize[2], 0)
			EndIf
      EndIf
